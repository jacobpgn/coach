  ...
  module Cli
    class ProviderFinder
      ...
      def find_provider
        ...
        if provider_mapping.has_key?(@value_name.to_sym)
      ...
      def find_chain
        ...
        if provider_chain.has_key?(@value_name.to_sym)
        ...
        if chains.size > 1
          puts "Value `#{@value_name}` is provided to `#{@middleware_name}` by multiple middleware chains:\n\n"
        else
          puts "Value `#{@value_name}` is provided to `#{@middleware_name}` by:\n\n"
        end
